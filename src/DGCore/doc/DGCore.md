...
<table>
	...
	<tr>
		...
		<td>Создает объект круга по переданной географической точке и необязательному объекту опций. Значение радиуса по умолчанию 10 пикселей.</td>
	</tr>
</table>
...
Используется для группировки нескольких слоев, чтобы обрабатывать их как один. Если группа добавлена на карту, тогда удалив элемент из группы он уаляется и с карты. Реализует интерфейс [ILayer][52].
...
Представляет слой [GeoJSON][95]. Позволяет анализировать данные в формате GeoJSON и отображать их на карте. Расширяет [FeatureGroup][26].
...
<table>
	...
	<tr id="geojson-filter">
		...
		<td>Функция, которая определяет отображать feature или нет.</td>
	</tr>
</table>
...
<table>
	...
	<tr>
		...
		<td><code><a href="#latlng">LatLng</a></code></td>
		<td>Создает объект LatLng по переданному масиву из двух чисел (широта, долгота), исользуется для GeoJSON точек. Если опция <code>reverse</code> установлена в <code>true</code>, тогда числа будут восприняты как (долгота, широта).</td>
	</tr>
	<tr>
