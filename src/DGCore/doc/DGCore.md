...
* [Для изменения состояний карты][4]
* [Для получения состояний карты][5]
* [Для слоев и элементов управления][6]
* [Методы конвертации][7]
* [Другие методы][8]
...
<table>
  <tr>
	...
	<tr>
		...
		<td>Колличество миллисекунд, которое должно пройти между остановкой движения карты и отпусканием кнопки мыши, для прекращения эффекта энерции. По умолчанию <code>32</code> для touch устройств и <code>14</code> для остальных.</td>
	</tr>
</table>
...
<table>
	...
	<tr>
		...
		<td>Устнавливает область просмотра карты (географический центр и зум). Если <code>forceReset</code> установлен в <code>true</code>, карта перегружается при движении и зуме (по умолчанию <code>false</code>).</td>
	</tr>
	<tr>
		<td><code><b>setZoom</b>(
			<nobr>&lt;Number&gt; <i>zoom</i> )</nobr>
	...
	<tr>
		...
		<td><code>this</code></td>
		<td>Увеличивает зум карты на величину <code>delta</code> (по умолчанию <code>1</code>).</td>
	</tr>
	<tr>
		<td><code><b>zoomOut</b>( <nobr>&lt;Number&gt; delta? )</nobr></code></td>
		<td><code>this</code></td>
		<td>Уменьшает зум карты на величину <code>delta</code> (по умолчанию <code>1</code>).</td>
	</tr>
	<tr id="map-fitbounds">
		...
		<td><code>this</code></td>
		<td>Устанавливает область просмотра карты так, чтобы та содержала заданные границы в максимально возможном уровне зума.</td>
	</tr>
	<tr id="map-fitworld">
	...
	<tr>
		...
		<td><code>this</code></td>
		<td>Проверяет изменился ли размер контейнера карты, если да, карта обновляется. Нужно вызвать этот метод, если размер контейнера изменился динамически. Если параметр <code>animate</code> установлен в <code>true</code>, обновление карты анимируется.</td>
	</tr>
	...
	<tr id="map-locate">
		...
		<td><code>this</code></td>
		<td>Пытается определить местоположения пользователя используя <a href="https://en.wikipedia.org/wiki/W3C_Geolocation_API">Geolocation API</a>. В случае успеха вызывается событие <code>locationfound</code> с данными о местонахождении, в случае ошибки <code>locationerror</code>, и опционально устанавливает область просмотра карты согласно местоположению пользователя (или просмотр карты мира, если возникла ошибка геолокации). Для дополнительной информации смотри <a href="#map-locate-options">Locate options</a>.</td>
	</tr>
	<tr>
...
<table>
	...
	<tr>
		...
		<td>Возвращает максимальный уровень зума при котором заданные границы полностью входят в область просмотра карты. Если опция <code>inside</code> установлена в  <code>true</code>, метод возвращает минимальный уровень зума с теми же условиями.</td>
	</tr>
	<tr>
...
<table>
	...
	<tr>
		...
		<td><code><a href="#point">Point</a></code></td>
		<td>Возвращает точку на карте которая соотвествует переданным координатам (полезно при размещении дополнительных слоев на карте).</td>
	</tr>
	<tr>
		...
		<td><code><a href="#latlng">LatLng</a></code></td>
		<td>Возвращает георграфические координаты переданной точки карты.</td>
	</tr>
	<tr>
	...
	<tr>
		...
		<td><code><a href="#point">Point</a></code></td>
		<td>Проецирует географические координаты в пиксельные для переданного уровня зума (текущий уровень зума используется по умолчанию).</td>
	</tr>
	<tr>
		...
		<td><code><a href="#latlng">LatLng</a></code></td>
		<td>Проецирует пиксельные координаты в географические для переданного уровня зума (по умолчанию используется текущий уровень зума).</td>
	</tr>
	<tr>
...
<table>
	...
	<tr>
		...
		<td><code>Boolean</code></td>
		<td><code>false</code></td>
		<td>Если <code>true</code>, постоянно отслеживает изменения локации (вместо определения локации однажны) используя W3C метод <code>watchPosition</code>. Можно остановить отслеживание вызвав метод <code><b>map.stopLocate</b>()</code>.</td>
	</tr>
	<tr>
		<td><code><b>setView</b></code></td>
		<td><code>Boolean</code></td>
		<td><code>false</code></td>
		<td>Если <code>true</code>, автоматически устанавливает область просмотра карты в точку локации пользователя, в соотвествии с точностью определения. В случае ошибки поиска, отображаетcя карта мира.</td>
	</tr>
	<tr>
	...
	<tr>
		...
		<td><code>Boolean</code></td>
		<td><code>false</code></td>
		<td>Включает повышенную точность, смотри <a href="http://dev.w3.org/geo/api/spec-source.html#high-accuracy">описание в W3C спецификации</a>.</td>
	</tr>
</table>
...
<table>
	...
	<tr>
		...
		<td><code>Boolean</code></td>
		<td><code>true</code></td>
		<td>Если значение <code>false</code>, обработчик клика по маркеру не вызывается.</td>
	</tr>
	<tr>
...
Свойства маркера включают в себя обработчики взаимодействия, которые позволяют конролировать интерактивное поведение, подключение и отключение определенных возможностей таких как драг (смотри методы [IHandler][51]). Например:
...
Используется для отображения балунов в определенных местах карты. Используйте [Map\#openPopup][72] для открытия балуна, одновременно открытым может быть только один балун, или [Map\#addLayer][81] для отображения любого колличества балунов.
...
<table>
	...
	<tr>
		...
		<td>Создает объект слоя тайлов по переданному <a href="#url-template">URL шаблону</a> и необязательному объекту опций.</td>
	</tr>
</table>
...
<table>
	...
	<tr>
		...
		<td><code>Boolean</code></td>
		<td><code>false</code></td>
		<td>Если установлено значение <code>true</code>, делает инверсию нумерации оси Y для тайлов (включите для TMS сервисов).</td>
	</tr>
	<tr>
		<td><code><b>continuousWorld</b></code></td>
		<td><code>Boolean</code></td>
		<td><code>false</code></td>
		<td>Если установлено значение <code>true</code>, координаты тайлов не будут "заворачиваться" по ширине мира (-180 до 180 широты) или высоте (-90 дл 90). Полезно, если вы используете Leaflet собственных карт, не отображающих реальный мир (т.е. игры, помещения или фото карты).</td>
	</tr>
	<tr>
		<td><code><b>noWrap</b></code></td>
		<td><code>Boolean</code></td>
		<td><code>false</code></td>
		<td>Если установлено значение <code>true</code>, тайлы не будут загружаться за пределами ширины мира (-180 до 180 широты).</td>
	</tr>
	<tr>
	...
	<tr>
		...
		<td><code>Boolean</code></td>
		<td><code>false</code></td>
		<td>Если установлено значение <code>true</code>, уровень зума указанный в URL тайла будет реверсирован (<code>maxZoom - zoom</code> вместо <code>zoom</code>)</td>
	</tr>
	<tr>
		<td><code><b>opacity</b></code></td>
		<td><code>Number</code></td>
	...
	<tr>
		...
		<td><code>Number</code></td>
		<td><code>null</code></td>
		<td>Указывает z-index тайлового слоя. Не установлен по умолчанию.</td>
	</tr>
	<tr>
		<td><code><b>unloadInvisibleTiles</b></code></td>
		<td><code>Boolean</code></td>
		<td>depends</td>
		<td>Если установлено значение <code>true</code>, все тайлы которые выходят за область видимости после любого перемещения карты удаляются (для лучшей производительности). <code>true</code> по умолчанию для мобильного WebKit, в остальных случаях <code>false</code>.</td>
	</tr>
	<tr>
		<td><code><b>updateWhenIdle</b></code></td>
		<td><code>Boolean</code></td>
		<td>depends</td>
		<td>Если установлено значение <code>false</code>, новые тайлы подгружаются во время перетягивания карты, иначе только по завершению перетягивания (для лучшей производительности). <code>true</code> по умолчанию для мобильного WebKit, в остальных случаях <code>false</code>.</td>
	</tr>
	<tr>
		<td><code><b>detectRetina</b></code></td>
		<td><code><code>Boolean</code></code></td>
		<td><code>false</code></td>
		<td>Если установлено значение <code>true</code> и у пользователя retina дисплей, вместо 1-го тайла будут загружены 4-е на большем уровне зума, также изображениям задается размер на 50% меньший их реального разрешения. Таким образом достигается лучшее качество отображения тайлов на мониторах с высоким разрешением.</td>
	</tr>
	<tr>
		<td><code><b>reuseTiles</b></code></td>
		<td><code><code>Boolean</code></code></td>
		<td><code>false</code></td>
		<td>Если установлено значение <code>true</code>, все тайлы которые невидны после перемешения карты добавляется в очередь переиспользования, из которой они будут взяты, если опять попадут в область видимости. В теории это сокращает объем используемой памяти.</td>
	</tr>
</table>
...
<table>
	...
	<tr>
		<td><code><b>mouseover</b></code></td>
		<td><code><a href="#mouse-event">MouseEvent</a></code>
		<td>Вызывается при наведении курсора мыши на объект.</td>
	</tr>
	<tr>
		<td><code><b>mouseout</b></code></td>
		<td><code><a href="#mouse-event">MouseEvent</a></code>
...
<table>
	...
	<tr>
		<td><code><b>mouseover</b></code></td>
		<td><code><a href="#mouse-event">MouseEvent</a></code>
		<td>Вызывается при наведении курсора мыши на группу.</td>
	</tr>
	<tr>
		<td><code><b>mouseout</b></code></td>
		<td><code><a href="#mouse-event">MouseEvent</a></code>
	...
	<tr>
		<td><code><b>mousemove</b></code></td>
		<td><code><a href="#mouse-event">MouseEvent</a></code>
		<td>Вызывается когда курсор мыши передвигается над группой.</td>
	</tr>
	<tr>
		<td><code><b>contextmenu</b></code></td>
		<td><code><a href="#mouse-event">MouseEvent</a></code>
...
Описывает методы для проецирования географических координат земной поверхности в плоские координаты (и обратно). См. [Картографическая проекция][118].
