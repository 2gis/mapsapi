...
В данном разделе рассматривается работа с векторными слоями, которые могут представлять собой геометрические 
объекты на карте: круги, ломаные линии, многоугольники.
...
<table id='dgpath-options'>
    ...
    <tbody>
        <tr id='path-stroke'>
            ...
            <td>
                Рисовать ли границу фигуры. Установите значение в <code>false</code>,
                чтобы отключить границы многоугольников или кругов.
            </td>
        </tr>
        <tr id='path-color'>
        ...
        <tr id='path-linecap'>
            ...
            <td>
                Строка, которая определяет
                <a target="_blank" href="https://developer.mozilla.org/en-US/docs/Web/SVG/Attribute/stroke-linecap">
                фигуру, которая будет использована как наконечник объекта</a>, если у нее задана граница (stroke).
            </td>
        </tr>
        <tr id='path-linejoin'>
            ...
            <td>
                ...
                фигуру, которая будет использована на углах изгибов</a> объекта, если у нее задана граница (stroke).
            </td>
        </tr>
        <tr id='path-dasharray'>
            ...
            <td>
                Строка <a target="_blank" href="https://developer.mozilla.org/en/SVG/Attribute/stroke-dasharray">шаблона границы</a>.
                Не работает на canvas слоях (например, Android 2).
            </td>
        </tr>
        <tr id='path-dashoffset'>
            ...
            <td>
                ...
                через которое начинается штриховка</a>. Не работает на canvas слоях (например, Android 2).
            </td>
        </tr>
        <tr id='path-fill'>
            ...
            <td>
                Заполнять ли фигуру цветом. Установите значение в <code>false</code>,
                чтобы отключить заполнение многоугольников или кругов.
            </td>
        </tr>
        <tr id='path-fillcolor'>
            ...
            <td><code>*</code></td>
            <td>
                Цвет заливки. Цвет по умолчанию определяется опцией <a href="#path-color"><code>color</code></a>.
            </td>
        </tr>
        ...
        <tr id='path-fillrule'>
            ...
            <td>
                Строка, которая определяет
                <a href="https://developer.mozilla.org/docs/Web/SVG/Attribute/fill-rule">что представляет собой
                внутренняя область фигуры</a>.
            </td>
        </tr>
        <tr id='path-interactive'>
            ...
            <td><code>true</code></td>
            <td>
                Если <code>false</code>, векторный слой не будет генерировать события мыши и будет
                вести себя как часть нижележащей карты.
            </td>
        </tr>
        <tr id='path-renderer'>
...
Класс для отрисовки ломаных линий на карте. Расширяет [Path](#dgpath).
...
<table id='dgpolyline-options'>
    ...
    <tbody>
        <tr id='polyline-smoothfactor'>
            ...
            <td><code>1.0</code></td>
            <td>
                Степень упрощения ломаной на каждом уровне масштаба. Большее значение означает выше
                производительность и меньшее качество, меньшее значение &mdash; лучшее качество,
...
<table id='dgpolyline-methods'>
    ...
    <tbody>
        <tr id='polyline-togeojson'>
            ...
            <td>
                Возвращает <a href="http://en.wikipedia.org/wiki/GeoJSON">GeoJSON</a>
                представление ломаной (GeoJSON LineString или <code>MultiLineString</code>).
            </td>
        </tr>
        ...
        <tr id='polyline-isempty'>
            ...
            <td><code>Boolean</code></td>
            <td><p>Возвращает <code>true</code>, если у Polyline нет точек LatLngs.</p></td>
        </tr>
        ...
        <tr id='polyline-addlatlng'>
            ...
            <td><code>this</code></td>
            <td>
                Добавляет данную точку в ломаную. По умолчанию добавляет точку в первое звено
                ...
                доступ ранее с помощью метода <a href="#polyline-getlatlngs">getlatlngs</a>).
            </td>
        </tr>
    </tbody>
</table>
...
Класс для отрисовки многоугольников на карте. Расширяет [Polyline](#dgpolyline).

Обратите внимание на то, что среди точек, которые передаются для создания многоугольника не должно быть
дополнительной точки, совпадающей с первой - такие точки лучше предварительно фильтровать.
...
Возможно также создать многоугольник с дырками, передав массив массивов latlngs,
...
Опции, унаследованные от [Polyline](#dgpolyline-options) <!-- TODO: include options -->
...
Методы, унаследованные от [Polyline](#dgpolyline-methods) <!-- TODO: include methods -->
...
Класс для отрисовки прямоугольников на карте. Расширяет [Polygon](#dgpolygon).
...
Методы, унаследованные от [Polygon](#dgpolygon-method) <!-- TODO: include methods -->
...
A class for drawing circle overlays on a map. Extends [CircleMarker](#dgcirclemarker). It's an approximation
and starts to diverge from a real circle closer to poles (due to projection distortion).

Класс для отрисовки круга на карте. Расширяет [Path](#dgpath). Это аппроксимация, которая начинает отличаться
от настоящего круга ближе к полюсам (из-за искажений проекции).
...
<table>
    ...
    <tbody>
        <tr id='circle-dg-circle'>
            ...
            <td>
                Создает объект круга по переданной географической точке и
                объекту опций, в котором содежится радиус круга.
            </td>
        </tr>
        <tr id='circle-dg-circle-2'>
            ...
            <td>
                Устаревший способ создания круга, только для совместимости со старым кодом.
                Не используйте его в своих новых приложениях и плагинах.
            </td>
        </tr>
    </tbody>
</table>
...
<table>
    ...
    <tbody>
        <tr id='circle-setradius'>
            ...
            <td><code>this</code></td>
            <td><p>Устанавливает радиус круга в метрах</p></td>
        </tr>
        <tr id='circle-getradius'>
            <td><code><b>getRadius</b>()</code></td>
            <td><code>Number</code></td>
            <td><p>Возвращает текущий радиус круга в метрах</p></td>
        </tr>
...
Методы, унаследованные от [CircleMarker](#dgcirclemarker-methods) <!-- TODO: include methods -->
...
<table>
    ...
    <tbody>
        <tr id='circlemarker-dg-circlemarker'>
            ...
            <td>Создает объект circleMarker по переданной географической точке и необязательному объекту опций.</td>
        </tr>
    </tbody>
</table>
...
<table id='dgcirclemarker-methods'>
    ...
    <tbody>
        <tr id='circlemarker-togeojson'>
            ...
            <td><code>Object</code></td>
            <td>
                Возвращает <a href="http://en.wikipedia.org/wiki/GeoJSON">GeoJSON</a> представление круглого маркера
                (GeoJSON Point).
            </td>
        </tr>
        ...
        <tr id='circlemarker-getradius'>
            <td><code><b>getRadius</b>()</code></td>
            <td><code>Number</code></td>
            <td>Возвращает текущее значение радиуса круга</td>
        </tr>
    </tbody>
</table>
...
Хотя SVG не доступен в IE8, в этом браузере есть поддержка
[VML](https://en.wikipedia.org/wiki/Vector_Markup_Language), и SVG renderer будет откарываться на
VML в этом случае. В 2012 году VML был признан устаревшим, это значит что VML в библиотеке существует
только для обратной совместимости со старой версией Internet Explorer.
...
<table>
    ...
    <tbody>
        <tr id='svg-create'>
            ...
            <td>
                Возвращает экземпляр <a href="https://developer.mozilla.org/docs/Web/API/SVGElement">SVGElement</a>,
                соответствуйщий имени класса, переданному как аргумент. Например, использование &#39;line&#39; вернет
                экземпляр <a href="https://developer.mozilla.org/docs/Web/API/SVGLineElement">SVGLineElement</a>.
            </td>
        </tr>
        <tr id='svg-pointstopath'>
            ...
            <td><code>String</code></td>
            <td>
                Генерирует строку SVG path для множества звеньев, каждое звено превращается
                в &quot;M..L..L..&quot;-инструкции 
            </td>
        </tr>
    </tbody>
...
Использование Canvas rendrer с дополнительными отступами для определенных векторных объектов:
...
<table>
    ...
    <tbody>
        <tr id='canvas-dg-canvas'>
            ...
            <td>Создает Canvas renderer с переданными опциями.</td>
        </tr>
    </tbody>
</table>
