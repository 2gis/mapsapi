...
Так как наш модуль предполагает взаимодействие пользователя с картой, мы его отнаследовали от класса `L.Handler`. Благодаря этому разработчик, который использует наш модуль, сможет контролировать его поведение в процессе исполнения приложения (например, у него будет возможность включить или выключить модуль).
