...
В связи с ограничением памяти в PhantomJS все тесты не могут выполниться за один раз.
Поэтому они выполняются по частям, по умолчанию в каждой части запускаются тесты из 10ти модулей.
Изменить это значение можно с помощью параметра: 

    --items-in-chunk

Если необходимо выполнить все тесты за один раз, то необходимо использовать параметр: 

    --one-chunk

Но при этом параметр --items-in-chunk будет игнорироваться.
