 ...
 * Layer main method
 */
L.FirmTileMarkers.Main = L.TileLayer.extend({
    ...
    statics: {
        url: 'http://catalog.api.2gis.ru/advanced',
        version: 1.3
    },
    ...
    /**
     * @type {L.FirmTileMarkers.ContainPointChecker}
     */
    _containPointChecker: null,

    /**
     * With help of this ID we can detect mousemove stopping
     * @type {String}
     */
    _mousemoveTimeoutId: null,

    /**
     * @type {Boolean}
     */
    _isMouseOverMarker: false,
    ...
    onAdd: function (map) {
        this._map = map;
        this._initContainer();
        this._createTileProto(); // create an image to clone for tiles
        this._createContainPointChecker();
    },

    /**
     * Send search request to 2GIS catalog and update tile layer
     * @param {String} what
     * @param {String} where
     */
    setRequest: function (what, where) {
        L.FirmTileMarkers.JSONP.get(
            L.FirmTileMarkers.Main.url,
            {
                criteria: '{"what":{"text":"' + what + '"},"where":{"text":"' + where + '"},"types":["firm"],"magic":{"geom2cache":"org_mini"}}',
                key: this.options.key,
                version: L.FirmTileMarkers.Main.version,
                output: 'jsonp'
            },
            L.Util.bind(this._onSearch, this)
        );
    },
    ...
    onRemove: function (map) {
        ...
        map.off({
            'mousemove': this._fireMoveEvents,
            'click': this._fireClickEvents
        }, this);

        this._lastSearchResults = null;
        this._containPointChecker = null;
    },
     ...
     */
    _onSearch: function (searchResults) {
        var tileHash = searchResults.results.firm.tilehash;
        this._containPointChecker.updateGeometries(searchResults);

        // append tile hash only after first search
        // subscribe to events only after first search
        if (this._lastSearchResults === null) {
            this.setUrl(this._url + tileHash + '/');
            ...
            this.setUrl(this._replaceTileHash(this._url, tileHash));
        }

        this._lastSearchResults = searchResults;
     ...
     */
    _update: function (e) {
        L.TileLayer.prototype._update.call(this, e);
        if (this._lastSearchResults !== null) {
            this._containPointChecker.updateGeometries(this._lastSearchResults);
        }
    },
     ...
     * @param {L.MouseEvent}
     */
    _fireMoveEvents: function (e) {
        var self = this;

        clearTimeout(this._mousemoveTimeoutId);
        this._mousemoveTimeoutId = setTimeout(function () {
            var checkResult = self._containPointChecker.check(e.layerPoint);
            if (checkResult !== false) {
                self._container.style.cursor = 'pointer';
                self._isMouseOverMarker = true;
                e.type = 'firm-tile-marker-mousemove';
                self._map.fire('firm-tile-marker-mousemove', e);
            }
            else if (self._isMouseOverMarker === true) {
                self._container.style.cursor = 'default';
                e.type = 'firm-tile-marker-mouseleave';
                self._map.fire('firm-tile-marker-mouseleave', e);
                self._isMouseOverMarker = false;
            }
        }, 10);
    },
     ...
     * @param {L.MouseEvent}
     */
    _fireClickEvents: function (e) {
        var checkResult = this._containPointChecker.check(e.layerPoint);
        if (checkResult !== false) {
            e.type = 'firm-tile-marker-click';
            this._map.fire('firm-tile-marker-click', e);
        }
    },

    /**
     * Point checker factory.
     * If you want to use markers of your own form, you must
     * write contain point strategy and describe (optional) geometry info.
     * This info will be added to each geometry and passed to your strategy.
     */
    _createContainPointChecker: function () {
        var strategy, shape, geometries, geometryInfo;

        switch (this.options.type) {
        case 'circle':
            strategy = new L.FirmTileMarkers.ContainPointCircleStrategy();
            geometryInfo = { radius: 6 };
            break;
        case 'your type':
            // construct yout strategy here
            break;
        }

        this._containPointChecker = new L.FirmTileMarkers.ContainPointChecker(strategy, this._map, geometryInfo);
    },
     ...
     */
    _replaceTileHash: function (currentUrl, newHash) {
        var urlParts = currentUrl.split('/');
        var currentHash = urlParts[urlParts.length - 2];
        var newUrl = currentUrl.replace(currentHash, newHash);
        return newUrl;
    }
});
...
/**
 * Usage:
 * L.FirmTileMarkers.JSONP.get( 'someUrl.php', {param1:'123', param2:'456'}, function(data){
 *   //do something with data, which is the JSON object you should retrieve from someUrl.php
 * });
 */
L.FirmTileMarkers.JSONP = (function () {
	...
	function jsonp(url, params, callback, callbackName) {
		...
		var jsp = "json" + (++counter);
		window[jsp] = function (data) {
			callback(data);
			try {
				delete window[jsp];
			} catch (e) {}
		...
		load(url + query + (callbackName || config.callbackName || 'callback') + '=' + jsp);
		return jsp;
	}
...
L.FirmTileMarkers.ContainPointChecker = L.Class.extend({
    /**
     * @type {IContainPointStrategy}
     */
    _strategy: null,
    ...
    /**
     * @param {IContainPointStrategy} strategy
     * @param {L.Map} map
     * @param {Object} geometryInfo Additional geometry info.
     * You can use this param if not enought information in searchResults,
     * for example you can pass radius info if each marker is round. Param is optional.
     */
    initialize: function (strategy, map, geometryInfo) {
        this._geomInfo = geometryInfo || {};
        this._strategy = strategy;
        this._map = map;
    },

    /**
     * Checks or geometry contains point via some strategy
     * @param {L.Point} point Point where the mouse event occured relative to the map layer
     * @return {Geometry|Boolean} geometry - contains point, false - not contains
     */
    check: function (point) {
        for (var i = this._geometries.length - 1; i >= 0; i--) {
            if (this._strategy.checkInclusion(this._geometries[i], point) === true) {
    ...
    /**
     * @param {Object} searchResults 2GIS catalog search results
     */
    updateGeometries: function (searchResults) {
        var self =  this;
        var results = searchResults.results.firm.magic.geometries;
        ...
        for (var i = results.length - 1; i >= 0; i--) {
            ...
            L.Util.extend(geometry, self._geomInfo);
            geometry.center = self._map.latLngToLayerPoint(latlng);
            self._geometries.push(geometry);
        }
    }
});

L.FirmTileMarkers.ContainPointCircleStrategy = L.Class.extend({
    /**
     * Checks or geometry contains point
     * @param {Object} geometry Example: {center: new L.Point(200, 300), radius: 10}
     * @param {L.Point} point
     * @return {Boolean} true - contains point, false - not contains
     */
    checkInclusion: function (geometry, point) {
        var dx, dy;
        
        dx = Math.abs(geometry.center.x - point.x);
        dy = Math.abs(geometry.center.y - point.y);
        
        if (dx >= geometry.radius || dy >= geometry.radius) {
            return false; // circle is in the rectangle
        }
        if (dx + dy <= geometry.radius) {
            return true; // rectangle is in the circle
        }
        
        return Math.pow(geometry.radius, 2) >= Math.pow(dx, 2) + Math.pow(dy, 2);
    }
});
